create DATABASE IF NOT EXISTS `madang` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;

-- create user from remote access
CREATE USER 'madang'@'%' IDENTIFIED BY 'madang';
-- add all previleges to user
GRANT ALL PRIVILEGES ON *.* TO 'madang'@'%' WITH GRANT OPTION;
...
insert into t1(name) values('hong');
insert into t1(id, name) values(2, 'park');
delete from t1 where name='choi';
SELECT * FROM `test`.`t1` LIMIT 1000;
